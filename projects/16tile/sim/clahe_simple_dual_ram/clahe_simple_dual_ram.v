...
`define IP_UUID _20f70a1c6a3f412f93093a4f75cf7630
`define IP_NAME_CONCAT(a,b) a``b
`define IP_MODULE_NAME(name) `IP_NAME_CONCAT(name,`IP_UUID)
module clahe_simple_dual_ram
    (
        input we,
        input [7:0] waddr,
        input [15:0] wdata_a,
...
module `IP_MODULE_NAME(bram_primitive) #(
    ...
    function integer address_map;
        ...
        case (index)
            0	:
                address_map=	(type_==0)?	1	:(type_==1)?	12	:(type_==2)?	13	:1;
            1	:
                address_map=	(type_==0)?	1	:(type_==1)?	12	:(type_==2)?	13	:1;
            2	:
                address_map=	(type_==0)?	2	:(type_==1)?	11	:(type_==2)?	12	:1;
            3	:
                address_map=	(type_==0)?	4	:(type_==1)?	10	:(type_==2)?	11	:1;
            4	:
                address_map=	(type_==0)?	4	:(type_==1)?	10	:(type_==2)?	11	:1;
            5	:
                address_map=	(type_==0)?	5	:(type_==1)?	10	:(type_==2)?	11	:1;
            6	:
                address_map=	(type_==0)?	8	:(type_==1)?	9	:(type_==2)?	10	:1;
            7	:
                address_map=	(type_==0)?	8	:(type_==1)?	9	:(type_==2)?	10	:1;
            8	:
                address_map=	(type_==0)?	8	:(type_==1)?	9	:(type_==2)?	10	:1;
            9	:
                address_map=	(type_==0)?	10	:(type_==1)?	9	:(type_==2)?	10	:1;
            10	:
                address_map=	(type_==0)?	10	:(type_==1)?	9	:(type_==2)?	10	:1;
            11	:
                address_map=	(type_==0)?	16	:(type_==1)?	8	:(type_==2)?	9	:1;
            12	:
                address_map=	(type_==0)?	16	:(type_==1)?	8	:(type_==2)?	9	:1;
            13	:
                address_map=	(type_==0)?	16	:(type_==1)?	8	:(type_==2)?	9	:1;
            ...
            16	:
                address_map=	(type_==0)?	16	:(type_==1)?	8	:(type_==2)?	9	:1;
            17	:
                address_map=	(type_==0)?	20	:(type_==1)?	8	:(type_==2)?	9	:1;
            18	:
                address_map=	(type_==0)?	20	:(type_==1)?	8	:(type_==2)?	9	:1;
            19	:
                address_map=	(type_==0)?	20	:(type_==1)?	8	:(type_==2)?	9	:1;
...
`include "bram_decompose.vh"
    ...
    function integer get_current_row_index;
        ...
        begin
            ...
            for (x=0; x<bram_mapping_size; x=x+1) begin
    ...
    generate
        ...
        if (rMux_mapping_B_size == 0) begin
            assign w_rdata_map = rMux[0];
        end
        else begin
            for (gen_y=0; gen_y<rMux_mapping_B_size; gen_y =gen_y +1) begin:rDataMux
        ...
        if (wen_sel_mapping_A_size!=0) begin
            for (gen_y=0; gen_y<wen_sel_mapping_A_size; gen_y =gen_y +1) begin:wDataEn
                ...
                if( BYPRASSED == 0 ) begin
                    ...
                    for (gen_x=SEL_START; gen_x<(SEL_END+1); gen_x = gen_x +1) begin
        ...
        if (data_mapping_table_A_size!=0) begin
            for (gen_y=0; gen_y<data_mapping_table_A_size; gen_y = gen_y +1) begin
        ...
        if (address_mapping_table_A_size!=0) begin
            for (gen_y=0; gen_y<address_mapping_table_A_size; gen_y = gen_y +1) begin
        ...
        if (data_mapping_table_B_size!=0) begin
            for (gen_y=0; gen_y<data_mapping_table_B_size; gen_y = gen_y +1) begin
        ...
        if (address_mapping_table_B_size!=0) begin
            for (gen_y=0; gen_y<address_mapping_table_B_size; gen_y = gen_y +1) begin
        ...
        if (bram_table_loop_mode == 0 ) begin:scan_column
            for (gen_x=0; gen_x<bram_table_size; gen_x =gen_x +1) begin:column
                ...
                for (gen_y=0; gen_y<row_count; gen_y =gen_y +1) begin:row
                    ...
                    for(gen_z=0;gen_z<WADDR_WIDTH_ROW; gen_z=gen_z+1) begin
                    ...
                    for(gen_z=0;gen_z<RADDR_WIDTH_ROW; gen_z=gen_z+1) begin
                    ...
                    if (WDATA_REPART == 0) begin
                        assign w_wdata[WDATA_WIDTH_ROW-1:0] = w_wdata_map[WDATA_END:WDATA_START];
                    end
                    else begin
                        //for Mixed Width Mode
                        for (gen_z=0; gen_z<WDATA_REPART; gen_z = gen_z+1) begin
                    ...
                    if (RDATA_REPART == 0) begin
                        assign rMux[RMUX_INDEX][RDATA_END:RDATA_START]   = w_rdata[RDATA_WIDTH_ROW-1:0];
                    end
                    else begin
                        //for Mixed Width Mode
                        for (gen_z=0; gen_z<RDATA_REPART; gen_z = gen_z+1) begin
        ...
        else if (bram_table_loop_mode == 1 ) begin:scan_row

            for (gen_y=0; gen_y<bram_table_size; gen_y =gen_y +1) begin:row
                ...
                for (gen_x=0; gen_x<column_count; gen_x =gen_x +1) begin:column
                    ...
                    if (WDATA_REPART == 0) begin
                        ...
                        //for Mixed Width Mode
                        for (gen_z=0; gen_z<WDATA_REPART; gen_z = gen_z+1) begin:MIXED_WIDTH_MAPPING
                    ...
                    if (RDATA_REPART == 0) begin
                        ...
                        //for Mixed Width Mode
                        for (gen_z=0; gen_z<RDATA_REPART; gen_z = gen_z+1) begin:MIXED_WIDTH_MAPPING
