...
module clahe_mapping_parallel #(
        ...
        input  wire        interp_enable,  // 插值使能信号（保留兼容）
        input  wire        cdf_ready,      // CDF准备就绪信号

        // ⚠️ Bug修复：4个独立的插值使能信号（从CLAHE_top传入）
        // 目的：阻止综合器常数传播优化，强制保持4个独立数据路径
        input  wire        interp_enable_tl,  // TL插值使能（UART动态初始化）
        input  wire        interp_enable_tr,  // TR插值使能（UART动态初始化）
        input  wire        interp_enable_bl,  // BL插值使能（UART动态初始化）
        input  wire        interp_enable_br,  // BR插值使能（UART动态初始化）
    ...
    //
    // ⚠️ Bug修复：使用4个独立使能信号（阻止综合器常数传播）
    // 原因：综合器将固定值1识别为编译时常数，导致错误的寄存器合并
    // 解决：通过UART动态初始化，使综合器无法进行常数传播优化
    // 逻辑：四个信号都来自外部动态控制，正常情况下都应该为1（开启插值）
    //       这样可以强制综合器保持四个独立的数据路径
    wire need_interp_tl = interp_enable_tl;  // 来自UART动态初始化（防止常数优化）
    wire need_interp_tr = interp_enable_tr;  // 来自UART动态初始化（防止常数优化）
    wire need_interp_bl = interp_enable_bl;  // 来自UART动态初始化（防止常数优化）
    wire need_interp_br = interp_enable_br;  // 来自UART动态初始化（防止常数优化）
    ...
    // 输出接口连接 - 使用4个独立使能信号
    // ========================================================================
    // 关键修复：每个tile使用独立的使能信号（来自UART动态初始化）
    // 目的：阻止综合器常数传播和错误的寄存器合并优化
    // 四块并行读取接口
    assign cdf_tl_tile_idx = need_interp_tl ? tile_idx_tl : tile_idx;
    assign cdf_tr_tile_idx = need_interp_tr ? tile_idx_tr : tile_idx;
    assign cdf_bl_tile_idx = need_interp_bl ? tile_idx_bl : tile_idx;
    assign cdf_br_tile_idx = need_interp_br ? tile_idx_br : tile_idx;
