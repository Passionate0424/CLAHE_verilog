// ============================================================================
// CLAHE 顶层模块 - 16 Tiles版本（支持YUV三路数据）
//
// 功能描述:
//   - 集成16块独立RAM架构（4×4 tiles），优化FPGA资源使用
//   - 支持YUV三路数据输入输出，确保数据对齐
//   - 保持原有的乒乓操作和流水线设计
//   - 完整的CLAHE处理流程，支持实时视频处理
//   - 四块并行读取功能，实现1像素/周期处理吞吐率
//   - 双线性插值功能，消除tile边界块效应
//
// 性能特点:
//   - Tile配置: 4×4 (16 tiles)
//   - Tile尺寸: 320×180 像素
//   - 清零时间: 256周期 (2.56μs @100MHz)
//   - 映射吞吐率: 1像素/周期
//   - 流水线级数: 5级
...
module clahe_top (
        ...
        // 控制接口 - 来自CPU或控制逻辑
        // ====================================================================
        input  wire [15:0] clip_threshold, // Clip限制值 (0-2047)，推荐值：450-900
        // 参考：平均值=225, clip_limit=225×factor
        // factor=2→450, factor=3→675, factor=4→900
        input  wire        enable_clahe,   // CLAHE使能信号，1=启用CLAHE处理
        input  wire        enable_interp,  // 插值使能信号，1=启用双线性插值

        // ⚠️ Bug修复：4个独立的插值使能信号（绕过综合器优化Bug）
        input  wire        enable_interp_tl,  // TL插值使能（通常连接到enable_interp）
        input  wire        enable_interp_tr,  // TR插值使能（通常连接到enable_interp）
        input  wire        enable_interp_bl,  // BL插值使能（通常连接到enable_interp）
        input  wire        enable_interp_br   // BR插值使能（通常连接到enable_interp）
    );
    ...
    localparam TILE_H = 4;          // 水平tile数量：4个（将图像分成4列）
    localparam TILE_V = 4;          // 垂直tile数量：4个（将图像分成4行）
    localparam TILE_NUM = 16;       // 总tile数量：4×4=16个（每个tile独立处理）
    localparam TILE_WIDTH = WIDTH / TILE_H;   // 每个tile宽度：320像素
    localparam TILE_HEIGHT = HEIGHT / TILE_V; // 每个tile高度：180像素
    ...
    wire [3:0]  tile_idx;               // tile索引 (0-15)，用于选择对应的RAM
    wire [8:0]  local_x;                // tile内相对X坐标 (0-319)
    wire [7:0]  local_y;                // tile内相对Y坐标 (0-179)
    ...
    // 直方图统计模块的RAM接口
    // ========================================================================
    wire [3:0]  hist_rd_tile_idx;    // 直方图统计读tile索引
    wire [3:0]  hist_wr_tile_idx;    // 直方图统计写tile索引
    ...
    // ========================================================================
    // 修复：分开定义读和写的tile索引
    wire [3:0]  cdf_rd_tile_idx;     // CDF读tile索引（读直方图）
    wire [3:0]  cdf_wr_tile_idx;     // CDF写tile索引（写LUT）
    ...
    // ========================================================================
    // 四块并行读取接口 - 从mapping模块输出
    wire [3:0]  mapping_tl_tile_idx_wire;    // 左上tile索引（mapping模块输出）
    wire [3:0]  mapping_tr_tile_idx_wire;    // 右上tile索引（mapping模块输出）
    wire [3:0]  mapping_bl_tile_idx_wire;    // 左下tile索引（mapping模块输出）
    wire [3:0]  mapping_br_tile_idx_wire;    // 右下tile索引（mapping模块输出）
    wire [7:0]  mapping_addr_wire;           // 映射地址（四块共用，mapping模块输出）

    // ============================================================
    // 最终方案：完全放弃隔离层，直接连线
    //
    // 失败历史：
    //   - 原方案（16×1-bit寄存器）：50%错误
    //   - 方案1（4×4-bit寄存器）：100%错误，23个bug
    //   - 方案5（模块实例化）：75%错误，17个bug，综合器展平模块层次
    //
    // 新策略：完全移除所有中间寄存器层
    //         - 直接将_wire信号连接到RAM模块
    //         - 让综合器"看不到"任何可优化的寄存器
    //         - 也许隔离层本身触发了bug
    //
    // 理论依据：如果没有中间寄存器，综合器无法进行"寄存器合并"优化
    // 风险评估：可能增加组合逻辑路径长度，但CLAHE已有充足时序裕量
    //
    // 日期：2025-11-04
    // ============================================================

    // 4个tile索引信号 - 直接连线，无寄存器
    wire [3:0] mapping_tl_tile_idx = mapping_tl_tile_idx_wire;  // 直通连接
    wire [3:0] mapping_tr_tile_idx = mapping_tr_tile_idx_wire;  // 直通连接
    wire [3:0] mapping_bl_tile_idx = mapping_bl_tile_idx_wire;  // 直通连接
    wire [3:0] mapping_br_tile_idx = mapping_br_tile_idx_wire;  // 直通连接

    reg [7:0] mapping_addr;  // 映射地址寄存器（保留）

    always @(posedge pclk or negedge rst_n) begin
        if (!rst_n)
            mapping_addr <= 8'd0;
        else
            mapping_addr <= mapping_addr_wire;
    end
    ...
    // ========================================================================
    // CDF LUT就绪标志：第一次CDF计算完成后保持为1
    // ========================================================================
    // 问题修复：第一帧时，mapping读取的RAM组还没有CDF数据（全0）
    // 解决：添加持续的就绪标志，只有在第一次CDF完成后才允许使用CDF增强
    reg cdf_lut_ready;
    ...
    // ========================================================================
    // 16块RAM实例（32块伪双端口RAM，16 RAM_A + 16 RAM_B）- 并行读取版本
    // ========================================================================
    ...
    clahe_mapping_parallel #(
                               ...
                               .interp_enable(enable_interp),  // 插值使能信号（兼容）
                               .cdf_ready(cdf_lut_ready),      // CDF准备就绪信号（持续标志，不是脉冲）

                               // Bug修复：传递4个独立的插值使能信号
                               .interp_enable_tl(enable_interp_tl),
                               .interp_enable_tr(enable_interp_tr),
                               .interp_enable_bl(enable_interp_bl),
                               .interp_enable_br(enable_interp_br),

                               // 四块并行CDF LUT读接口（连接到_wire信号）
                               .cdf_tl_tile_idx(mapping_tl_tile_idx_wire),     // 左上tile索引
                               .cdf_tr_tile_idx(mapping_tr_tile_idx_wire),     // 右上tile索引
                               .cdf_bl_tile_idx(mapping_bl_tile_idx_wire),     // 左下tile索引
                               .cdf_br_tile_idx(mapping_br_tile_idx_wire),     // 右下tile索引
                               .cdf_rd_bin_addr(mapping_addr_wire),            // 读bin地址
    ...
    // RAM接口架构说明（并行读取版本）
    // ========================================================================
    // 本设计使用32块伪双端口RAM实现乒乓操作：
    //   - 16块RAM_A：当ping_pong_flag=0时用于统计，ping_pong_flag=1时用于映射
    //   - 16块RAM_B：当ping_pong_flag=1时用于统计，ping_pong_flag=0时用于映射
    //
    // 各模块的RAM访问：
    ...
    //   - 高吞吐率：实现1像素/周期的处理能力
    //
    // 端口仲裁已在clahe_ram_16tiles_parallel模块内部实现，无需顶层仲裁逻辑
    ...
    wire [3:0]  debug_tile_idx = tile_idx;
    wire [8:0]  debug_local_x = local_x;
    ...
    wire        debug_cdf_ready = cdf_lut_ready;  // 修改：使用持续标志而不是脉冲
